...
// Larger than Life
bool largerThanLifeMode = false;
bool inLargerThanLifeSubMenu = false;
uint8_t largerThanLifeSelection = 1;
uint8_t currentLargerThanLifeRule = 1;
bool largerThanLifeColorMode = false;
bool largerThanLifeSavedColorMode = false;

// LtL uses larger neighborhoods - we'll use 5x5 (range 2)
int ltl_range = 2;  // 2 = 5x5, 3 = 7x7, 4 = 9x9
int ltl_birthMin = 34;
int ltl_birthMax = 45;
int ltl_surviveMin = 34;
int ltl_surviveMax = 58;
...
void initializeLargerThanLife();
void updateLargerThanLife();
void drawLargerThanLife();
void setLargerThanLifeRule(int ruleNum);
...
void changeCellSize() {
    ...
    if (gameMode == 1 || ruleExplorerMode) {
        ...
        // Redraw grid AFTER cells to ensure it's visible
        if (showGridLines) {
            redrawGridOverCells();
        }
        generation = 0;
    ...
    } else if (gameMode == 10 && largerThanLifeMode) {
        // Larger than Life
        initializeLargerThanLife();
        drawLargerThanLife();
        // Redraw grid AFTER cells
        if (showGridLines) {
            redrawGridOverCells();
...
void handleButtonB() {
    ...
    if (!inMenuMode && !editMode) {
        ...
        if (!buttonBPressed && buttonBWasPressed) {
            ...
            if (largerThanLifeMode) {
                largerThanLifeMode = false;
                largerThanLifeColorMode = false;
                editMode = false;
                showingStats = false;
                inMenuMode = true;
                inLargerThanLifeSubMenu = true;
                inAltGamesSubMenu = false;
                largerThanLifeSelection = currentLargerThanLifeRule;
    ...
    if (!buttonBPressed && buttonBWasPressed) {
        ...
        if (inLargerThanLifeSubMenu) {
            display.fillScreen(COLOR_BLACK);
            inLargerThanLifeSubMenu = false;
            inAltGamesSubMenu = true;
            altGameSelection = 5;
            showMenu();
            return;
       ...
       if (largerThanLifeMode) {
                largerThanLifeSavedColorMode = largerThanLifeColorMode;  // SAVE color preference
                largerThanLifeMode = false;
                largerThanLifeColorMode = false;  // Reset for now
                editMode = false;
                showingStats = false;
                inMenuMode = true;
                inLargerThanLifeSubMenu = true;
    ...
    if (!currentlyPressed && setButtonPressed) {
        ...
        if (inMenuMode && pressDuration < 800) {
            ...
            if (!inPatternsSubMenu && !inSymmetricSubMenu && !inAltGamesSubMenu && 
                !inBriansBrainSubMenu && !inSeedsSubMenu && !inCyclicSubMenu && !inDayNightSubMenu && !inCellSizeSubMenu && !inToolsSubMenu && !inLargerThanLifeSubMenu) {
            ...
            if (inLargerThanLifeSubMenu) {
    gameMode = 10;
    largerThanLifeMode = true;
    largerThanLifeColorMode = largerThanLifeSavedColorMode;  // ADD THIS LINE - restore saved color preference
    currentLargerThanLifeRule = largerThanLifeSelection;
    
    setLargerThanLifeRule(largerThanLifeSelection);
...
void loop() {
    ...
    if (!editMode) {
        ...
        if (showingCellSizeIndicator && (current_time - cellSizeIndicatorTime >= 2000)) {
            ...
            // Clear the indicator area (black rectangle + text area)
            display.fillRect(SCREEN_WIDTH - 60, 5, 55, 20, COLOR_BLACK);
            
            // Redraw affected cells in that area
            int startX = (SCREEN_WIDTH - 60) / currentCellSize;
            int endX = SCREEN_WIDTH / currentCellSize;
            int startY = 5 / currentCellSize;
            int endY = 25 / currentCellSize;
            
            for (int x = startX; x <= endX && x < BOARD_WIDTH; x++) {
                for (int y = startY; y <= endY && y < BOARD_HEIGHT; y++) {
                    if (x >= 0 && x < BOARD_WIDTH && y >= 0 && y < BOARD_HEIGHT) {
                        // Determine which game mode and color mode
                        if (briansBrainMode) {
                            // Redraw Brian's Brain cells
                            bool isAlive = board[x][y];
                            bool isDying = briansBrain_dying[x][y];
                            
                            if (briansBrainColorMode) {
                                if (isAlive && !isDying) {
                                    drawCell(x, y, 0x07FF); // Cyan
                                } else if (isDying) {
                                    uint8_t age = cellAges[x][y]; // Using cellAges for trail
                                    if (age <= 3) drawCell(x, y, 0xFFFF);
                                    else if (age <= 6) drawCell(x, y, 0xFFE0);
                                    else if (age <= 9) drawCell(x, y, 0xFD20);
                                    else if (age <= 12) drawCell(x, y, 0xF800);
                                    else if (age <= 15) drawCell(x, y, 0x780F);
                                    else if (age <= 18) drawCell(x, y, 0x4008);
                                    else drawCell(x, y, COLOR_BLACK);
                                } else {
                                    drawCell(x, y, COLOR_BLACK);
                                }
                            } else {
                                // Mono mode - don't redraw (blinking handled elsewhere)
                                if (isAlive && !isDying) {
                                    drawCell(x, y, COLOR_WHITE);
                                } else {
                                    drawCell(x, y, COLOR_BLACK);
                            ...
                            if (seedsColorMode) {
                                ...
                                if (age == 0 && board[x][y]) drawCell(x, y, 0xFFFF);
                                else if (age == 1) drawCell(x, y, COLOR_BLACK);
                                else if (age == 2) drawCell(x, y, 0x07FF);
                                else if (age == 3) drawCell(x, y, COLOR_BLACK);
                                else if (age == 4) drawCell(x, y, 0x001F);
                                else if (age == 5) drawCell(x, y, 0x780F);
                                else if (age == 6) drawCell(x, y, 0xF81F);
                                else if (age == 7) drawCell(x, y, 0xF800);
                                else if (age == 8) drawCell(x, y, 0xFD20);
                                else if (age == 9) drawCell(x, y, 0xFFE0);
                                else if (age == 10) drawCell(x, y, 0x8410);
                                else if (age == 11) drawCell(x, y, 0x2104);
                                else drawCell(x, y, COLOR_BLACK);
                            } else {
                                drawCell(x, y, board[x][y] ? COLOR_WHITE : COLOR_BLACK);
                            }
                            ...
                            if (cyclicColorMode) {
                                ...
                                switch(state) {
                                    ...
                                    case 3: color = 0x07E0; break;
                                    case 4: color = 0x07FF; break;
                                    case 5: color = 0xF81F; break;
                                }
                                drawCell(x, y, color);
                            } else {
                                // Mono mode - just clear to black
                                drawCell(x, y, COLOR_BLACK);
                            }
                        } else if (largerThanLifeMode) {
                            if (largerThanLifeColorMode) {
                                if (board[x][y]) {
                                    drawCell(x, y, getColorByAge(cellAges[x][y]));
                                } else {
                                    drawCell(x, y, COLOR_BLACK);
                        ...
                        } else if (ruleExplorerMode || (gameMode >= 1 && gameMode <= 4)) {
                            // Standard Conway's Life modes
                            if (board[x][y]) {
                                if (colorMode) {
                                    drawCell(x, y, getColorByAge(cellAges[x][y]));
                                } else {
            ...
            // Redraw grid over the cleared area if enabled
            if (showGridLines && currentCellSize >= 3) {
                for (int x = startX; x <= endX && x <= BOARD_WIDTH; x++) {
                    display.drawFastVLine(x * currentCellSize, 5, 20, 0x2104);
                }
                for (int y = startY; y <= endY && y <= BOARD_HEIGHT; y++) {
                    display.drawFastHLine(SCREEN_WIDTH - 60, y * currentCellSize, 60, 0x2104);
                }
            }
        }
...
void setLargerThanLifeRule(int ruleNum) {
    switch(ruleNum) {
        case 1: // Bugs - 5x5 neighborhood (24 neighbors max)
            ltl_range = 2;
            ltl_birthMin = 5;
            ltl_birthMax = 9;
            ltl_surviveMin = 5;
        ...
        case 2: // Glob - 7x7 neighborhood (48 neighbors max)
            ltl_range = 3;
            ltl_birthMin = 10;
            ltl_birthMax = 18;
            ltl_surviveMin = 15;
        ...
        case 3: // Bosco's Rule - 5x5 neighborhood (24 neighbors max)
            ltl_range = 2;
            ltl_birthMin = 6;
            ltl_birthMax = 12;
            ltl_surviveMin = 8;
        ...
        case 4: // Random
            ltl_range = 2 + random(2);  // 5x5 or 7x7
            int maxNeighbors = (2 * ltl_range + 1) * (2 * ltl_range + 1) - 1;
            ltl_birthMin = random(maxNeighbors / 3, maxNeighbors / 2);
            ltl_birthMax = ltl_birthMin + random(5, 15);
            ltl_surviveMin = random(maxNeighbors / 3, maxNeighbors / 2);
            ltl_surviveMax = ltl_surviveMin + random(10, 25);
            break;
    }
}
...
void initializeLargerThanLife() {
    ...
    int size = (currentCellSize == 2) ? 20 : 15;
    
    // Create a dense blob in the center
    for (int x = centerX - size; x <= centerX + size; x++) {
        for (int y = centerY - size; y <= centerY + size; y++) {
            if (x >= 0 && x < BOARD_WIDTH && y >= 0 && y < BOARD_HEIGHT) {
                ...
                // Denser in center, with some randomness
                if (dist < size) {
                    if (random(100) < (100 * (1.0 - dist/size))) {
                        board[x][y] = true;
                        cellAges[x][y] = 1;
                    }
                }
...
void updateLargerThanLife() {
    ...
    for (int x = 0; x < BOARD_WIDTH; x++) {
        for (int y = 0; y < BOARD_HEIGHT; y++) {
            ...
            // Count neighbors in larger neighborhood
            for (int dx = -ltl_range; dx <= ltl_range; dx++) {
                for (int dy = -ltl_range; dy <= ltl_range; dy++) {
                    if (dx == 0 && dy == 0) continue;
            ...
            if (board[x][y]) {
                // Cell is alive
                newBoard[x][y] = (neighbors >= ltl_surviveMin && neighbors <= ltl_surviveMax);
            } else {
                // Cell is dead
                newBoard[x][y] = (neighbors >= ltl_birthMin && neighbors <= ltl_birthMax);
            }
...
void drawLargerThanLife() {
    ...
    // Reset firstDraw if board dimensions changed or generation reset
    if (BOARD_WIDTH != lastBoardWidth || BOARD_HEIGHT != lastBoardHeight || generation < lastGeneration) {
        firstDraw = true;
    ...
    // Draw only changed cells by comparing board with lastBoard
    for (int x = 0; x < BOARD_WIDTH; x++) {
        for (int y = 0; y < BOARD_HEIGHT; y++) {
            bool changed = (board[x][y] != lastBoard[x][y]);
            ...
            if (changed) {
                ...
                lastBoard[x][y] = board[x][y];
            } else if (largerThanLifeColorMode && board[x][y]) {
                // Update color for aging cells in color mode
                drawCell(x, y, getColorByAge(cellAges[x][y]));
            }
        }
    }
...
void drawCellWithGrid(int x, int y, uint16_t color) {
    ...
    if (showGridLines && currentCellSize >= 3) {
        // Draw cell interior (leaving 1-pixel border for grid on top and left)
        // The grid is drawn as a complete overlay, so we just avoid the border pixels
        if (color == COLOR_BLACK) {
            // For black cells, fill interior only (grid remains visible)
            display.fillRect(x * currentCellSize + 1, y * currentCellSize + 1, 
                            currentCellSize - 1, currentCellSize - 1, COLOR_BLACK);
        } else {
            // For colored cells, fill interior only
            display.fillRect(x * currentCellSize + 1, y * currentCellSize + 1, 
                            currentCellSize - 1, currentCellSize - 1, color);
        }
        // DON'T redraw grid lines here - they're already drawn by drawFullGrid()
        // Redrawing them for each cell is inefficient and causes the issue
    } else {
...
void redrawGridOverCells() {
    ...
    // Redraw grid lines over all cells
    // Draw vertical lines
    for (int x = 0; x <= BOARD_WIDTH; x++) {
        display.drawFastVLine(x * currentCellSize, 0, SCREEN_HEIGHT, 0x2104);
    }
...
void updateSeeds() {
  ...
  void drawSeeds() {
    ...
    if (seedsColorMode) {
        // COLOR MODE - Trail/fade effect - draw all cells every frame
        for (int x = 0; x < BOARD_WIDTH; x++) {
            for (int y = 0; y < BOARD_HEIGHT; y++) {
                board[x][y] = newBoard[x][y];
        ...
        // MONO MODE - draw only changed cells
        for (int x = 0; x < BOARD_WIDTH; x++) {
            for (int y = 0; y < BOARD_HEIGHT; y++) {
                if (board[x][y] != newBoard[x][y]) {
                    board[x][y] = newBoard[x][y];
...
void drawCyclic() {
    ...
    if (cyclicColorMode) {
        // COLOR MODE - Rainbow cycle - just redraw everything
        for (int x = 0; x < BOARD_WIDTH; x++) {
            for (int y = 0; y < BOARD_HEIGHT; y++) {
                uint8_t state = cyclicStates[x][y];
                uint16_t color;
        ...
        // Redraw everything each frame - simpler
        for (int x = 0; x < BOARD_WIDTH; x++) {
            for (int y = 0; y < BOARD_HEIGHT; y++) {
                uint8_t state = cyclicStates[x][y];
...
if (inToolsSubMenu) numItems = 6;  // Changed from 3 to 6
    ...
    else if (inAltGamesSubMenu) numItems = 5;
    else if (inBriansBrainSubMenu) numItems = 5;
    else if (inCyclicSubMenu) numItems = 3;
    else if (inLargerThanLifeSubMenu) numItems = 4;
    ...
    for (int i = 1; i <= numItems; i++) {
        ...
        if (inToolsSubMenu) {
            ...
            else if (i == 4) display.println("Cyclic");
            else if (i == 5) display.println("Larger than Life");
            ...
            } else if (inLargerThanLifeSubMenu) {
            if (i == 1) display.println("Bugs");
            else if (i == 2) display.println("Glob");
            else if (i == 3) display.println("Bosco's Rule");
            else display.println("Random");
...
void showMenu() {
    ...
    if (inToolsSubMenu) {
        ...
        display.println("LARGER THAN LIFE");
    } else if (inDayNightSubMenu) {
...
int yPos = 50;
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        switch(i) {
            ...
            case 5: display.println("Larger than Life"); break;
        }
        yPos += 28;
    }
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            display.setCursor(10, yPos + 5);
            switch(i) {
                case 1: display.println("Bugs"); break;
                case 2: display.println("Glob"); break;
                case 3: display.println("Bosco's Rule"); break;
                case 4: display.println("Random"); break;
            }
            yPos += 28;
        }
...
void showGameRules() {
    ...
    if (ruleExplorerMode) {
        ...
        display.println(F("LARGER THAN LIFE"));
        display.setCursor(20, 60);
        display.print(F("Range: "));
        display.println(ltl_range);
        display.setCursor(20, 90);
        display.print(F("Birth: "));
        display.print(ltl_birthMin);
        display.print(F("-"));
        display.println(ltl_birthMax);
        display.setCursor(20, 120);
        display.print(F("Survive: "));
        display.print(ltl_surviveMin);
        display.print(F("-"));
        display.println(ltl_surviveMax);
    } else if (gameMode >= 1 && gameMode <= 4) {
...
void handleDirectionalButtons() {
  ...
  // CHECK FOR DAY & NIGHT COLOR MODE TOGGLE (UP + DOWN together)
    ...
    // CHECK FOR LARGER THAN LIFE COLOR MODE TOGGLE (UP + DOWN together)
if (!inMenuMode && !editMode && largerThanLifeMode) {
    if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
        lastColorToggle = now;
        largerThanLifeColorMode = !largerThanLifeColorMode;
        largerThanLifeSavedColorMode = largerThanLifeColorMode;  // ADD THIS LINE - save immediately
        playColorToggle(largerThanLifeColorMode);
            ...
            if (now - lastUpPress >= 150) {
                ...
                int oldSel, newSel;
                 ...
                 if (inToolsSubMenu) {
                    ...
                    altGameSelection = (altGameSelection > 1) ? altGameSelection - 1 : 5;
                    ...
                    oldSel = largerThanLifeSelection;
                    largerThanLifeSelection = (largerThanLifeSelection > 1) ? largerThanLifeSelection - 1 : 4;
                    newSel = largerThanLifeSelection;
                    // Don't call updateMenuSelection - just redraw the menu
                    showMenu();
                } else if (inDayNightSubMenu) {
        ...
        if (isBtnDown()) {
            if (now - lastDownPress >= 150) {
                ...
                if (inToolsSubMenu) {
                    ...
                    oldSel = altGameSelection;
                    altGameSelection = (altGameSelection < 5) ? altGameSelection + 1 : 1;
                    newSel = altGameSelection;
                    updateMenuSelection(oldSel, newSel);
                } else if (inBriansBrainSubMenu) {
                    oldSel = briansBrainSizeSelection;
                    ...
                    oldSel = largerThanLifeSelection;
                    largerThanLifeSelection = (largerThanLifeSelection < 4) ? largerThanLifeSelection + 1 : 1;
                    newSel = largerThanLifeSelection;
                    // Don't call updateMenuSelection - just redraw the menu
                    showMenu();
                } else if (inDayNightSubMenu) {
...
if (isBtnDown() && !isBtnUp() && (now - lastDownPress >= 150)) {
    ...
    if (isBtnRight() && !isBtnLeft() && (now - lastRightPress >= 250)) {
        ...
        if (gameMode == 1 || ruleExplorerMode) {
            ...
            // If on "Random" rule, generate new random parameters
            if (currentLargerThanLifeRule == 4) {
                setLargerThanLifeRule(4);  // Re-randomize
            }
            ...
            initializeLargerThanLife();
            drawLargerThanLife();  // Draw with current color mode
