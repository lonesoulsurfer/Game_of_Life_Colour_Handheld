...
const Rule PRESET_RULES[] PROGMEM = {
    {"Conway",    0b00001000, 0b00001100},  // B3/S23
    {"HighLife",  0b01001000, 0b00001100},  // B36/S23
    {"Maze",      0b00001000, 0b00111110},  // B3/S12345
    {"Coral",     0b00001000, 0b11111000},  // B3/S45678
    {"Seeds",     0b00000100, 0b00000000},  // B2/S
    {"Replicatr", 0b10101010, 0b10101010},  // B1357/S1357
    {"2x2",       0b01001000, 0b00100010},  // B36/S125
    {"NoDeath",   0b00001000, 0b11111111},  // B3/S012345678
    {"Diamoeba",  0b11111000, 0b11111000},  // B35678/S5678
};
const int NUM_PRESET_RULES = 9;
...
void changeCellSize() {
    ...
    if (gameMode == 1) {
        ...
        // Brian's Brain
        display.fillScreen(COLOR_BLACK);  // Clear screen FIRST
    ...
    if (!currentlyPressed && setButtonPressed) {
        ...
        if (inMenuMode && pressDuration < 800) {
            ...
            if (!inPatternsSubMenu && !inSymmetricSubMenu && !inAltGamesSubMenu && 
                !inBriansBrainSubMenu && !inSeedsSubMenu && !inCyclicSubMenu && !inDayNightSubMenu && !inCellSizeSubMenu) {
...
delay(1000);  // Show pattern for 1 second (changed from 2000)
...
// Run animation until all cells leave the screen or max generations
    ...
    for (int gen = 0; gen < maxGenerations; gen++) {
        ...
        // Visible screen is 27x20 cells, so check if all cells are outside that area
...
void loop() {
    ...
    if (current_time - statsStartTime >= 4000) {
        showingStats = false;
                ...
                if (briansBrainMode) {
                    display.fillScreen(COLOR_BLACK);  // ADD THIS
                    ...
                    // Go back to appropriate menu based on game mode
                    if (gameMode == 2) {
                        inMenuMode = true;
                        inCellSizeSubMenu = true;
                        cellSizeSelection = (currentCellSize == 2) ? 1 : ((currentCellSize == 3) ? 2 : ((currentCellSize == 8) ? 4 : 3));
...
void drawBriansBrain() {
    ...
    static uint32_t lastGeneration = 0;  // ADD THIS
...
void updateMenuSelection(int oldSelection, int newSelection) {
    ...
    int numItems = 6; // Default for main menu
    if (inSymmetricSubMenu) numItems = 3;
    ...
    for (int i = 1; i <= numItems; i++) {
        ...
        if (inSymmetricSubMenu) {
            ...
            if (i == 1) display.println("VrtclSym");
            else if (i == 2) display.println("4WayRot");
            ...
            else if (i == 3) display.println("Symmetrc");
            else if (i == 4) display.println("Custom");
            else if (i == 5) display.println("AltGame");
            else display.println("RuleExplr");
        }
...
void showRuleExplorerMenu() {
    ...
    if (inCustomRuleEdit) {
        ...
        if (totalItems > maxVisibleItems) {
            ...
            if (scrollOffset + maxVisibleItems < totalItems) display.print("v");
        }
    }
}
...
int yPos = 50;
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            display.setCursor(10, yPos + 5);
            switch(i) {
                case 1: display.println("VrtclSym"); break;
                case 2: display.println("4WayRot"); break;
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            switch(i) {
                ...
                case 3: display.println("Symmetrc"); break;
                case 4: display.println("Custom"); break;
                case 5: display.println("AltGame"); break;
                case 6: display.println("RuleExplr"); break;
            }
            yPos += 28;
        }
    } else if (inCellSizeSubMenu) {
...
void handleDirectionalButtons() {
    ...
    if (!inMenuMode && !editMode && (gameMode >= 1 && gameMode <= 4)) {
        if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
            ...
            display.setTextColor(COLOR_WHITE);
            display.setCursor(SCREEN_WIDTH - 58, 10);
            display.print(colorMode ? "COLOR" : "MONO");
            
            // Set timer for indicator
            colorIndicatorTime = now;
            showingColorIndicator = true;
    ...
    if (!inMenuMode && !editMode && cyclicMode) {
        if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
            ...
            display.setTextColor(COLOR_WHITE);
            display.setCursor(SCREEN_WIDTH - 58, 10);
            display.print(cyclicColorMode ? "COLOR" : "MONO");
    ...
    if (!inMenuMode && !editMode && seedsMode) {
        if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
            ...
            display.setTextColor(COLOR_WHITE);
            display.setCursor(SCREEN_WIDTH - 58, 10);
            display.print(seedsColorMode ? "COLOR" : "MONO");
  ...
  // CHECK FOR DAY & NIGHT COLOR MODE TOGGLE (UP + DOWN together)
    if (!inMenuMode && !editMode && dayNightMode) {
        if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
            ...
            display.setTextColor(COLOR_WHITE);
            display.setCursor(SCREEN_WIDTH - 58, 10);
            display.print(dayNightColorMode ? "COLOR" : "MONO");
    ...
    if (!inMenuMode && !editMode && briansBrainMode) {
        if (isBtnUp() && isBtnDown() && (now - lastColorToggle >= 500)) {
            ...
            display.setTextColor(COLOR_WHITE);
            display.setCursor(SCREEN_WIDTH - 58, 10);
            display.print(briansBrainColorMode ? "COLOR" : "MONO");
    ...
    if (inMenuMode) {
        ...
        if (isBtnUp()) {
            if (now - lastUpPress >= 150) {
                lastUpPress = now;
                flashLED();  // Flash FIRST to confirm button detected
                ...
                if (inSymmetricSubMenu) {
                    ...
                    menuSelection = (menuSelection > 1) ? menuSelection - 1 : 6;
        ...
        if (isBtnDown()) {
            if (now - lastDownPress >= 150) {
                ...
                if (inSymmetricSubMenu) {
                    ...
                    oldSel = menuSelection;
                    menuSelection = (menuSelection < 6) ? menuSelection + 1 : 1;
                    newSel = menuSelection;
                    updateMenuSelection(oldSel, newSel);
                }
            }
    ...
    // Edit mode handling
    if (editMode) {
        if (isBtnUp() && (now - lastUpPress >= 120)) {
            lastUpPress = now;
            cursorY--;
        ...
        if (isBtnDown() && (now - lastDownPress >= 120)) {
            lastDownPress = now;
            cursorY++;
        ...
        if (isBtnLeft() && (now - lastLeftPress >= 120)) {
            lastLeftPress = now;
            cursorX--;
        ...
        if (isBtnRight() && (now - lastRightPress >= 120)) {
            lastRightPress = now;
            cursorX++;
    ...
    if (isBtnRight() && !isBtnLeft() && (now - lastRightPress >= 250)) {
        ...
        if (gameMode == 1 || ruleExplorerMode) {
          ...
          showingStats = false;
            ...
            showingStats = false;  // ADD THIS - clear stats flag
            display.fillScreen(COLOR_BLACK);
            clearBoard();  // This clears ALL arrays including dying cells
            ...
            if (briansBrainWasCustom) {
                ...
                generation = 0;
                cellCount = 0;
                max_cellCount = 0;  // ADD THIS - reset max too
                briansBrain_lowCellCounter = 0;  // ADD THIS - reset low cell counter
                frame_start_t = millis();
                start_t = micros();  // ADD THIS - reset start time
                drawBriansBrain();
            }
            flashLED();
        }
