        ...
        if (inMenuMode && pressDuration < 800) {
            ...
            if (inToolsSubMenu) {
                ...
                if (toolsSelection == 1) {
                    ...
                    playMenuBeep();
                    showMenu();  // Redraw to show updated toggle
                    buttonProcessingInProgress = false;
                    // Add delay to prevent double-press
                    delay(200);
                    return;
...
void playStartupTune() {
    ...
    // Opening phrase - "awakening" (during person pattern display)
    playTone(523, 150);   // C5
    delay(10);
    playTone(659, 150);   // E5
    delay(10);
    ...
    // Rising phrase - "evolution" (during animation)
    playTone(784, 100);   // G5
    delay(10);
    playTone(880, 100);   // A5
    delay(10);
    ...
    // Triumphant finish - "life!" (during title reveal)
    playTone(1047, 150);  // C6
    delay(10);
    playTone(1319, 150);  // E6
    delay(10);
    playTone(1568, 300);  // G6 - hold longer for emphasis
}
...
// Keep person visible for 1 second
delay(1000);
...
// Run animation until all cells leave the screen or max generations
    ...
    for (int gen = 0; gen < maxGenerations; gen++) {
        ...
        // Play Jurassic Park theme during animation
        if (soundEnabled && gen % 8 == 0) {  // Play a note every 8 frames (slower)
            switch(musicNote) {
                case 0: tone(BUZZER_PIN, 784, 400); break;   // G5 - longer
                case 1: tone(BUZZER_PIN, 880, 350); break;   // A5
                case 2: tone(BUZZER_PIN, 988, 300); break;   // B5
                case 3: tone(BUZZER_PIN, 1047, 700); break;  // C6 - hold
                case 4: tone(BUZZER_PIN, 988, 400); break;   // B5
                case 5: tone(BUZZER_PIN, 1047, 800); break;  // C6 - longer hold
                case 6: tone(BUZZER_PIN, 659, 350); break;   // E5
                case 7: tone(BUZZER_PIN, 784, 350); break;   // G5
                case 8: tone(BUZZER_PIN, 880, 350); break;   // A5
                case 9: tone(BUZZER_PIN, 988, 350); break;   // B5
                case 10: tone(BUZZER_PIN, 1047, 500); break; // C6
                case 11: tone(BUZZER_PIN, 1175, 500); break; // D6
                case 12: tone(BUZZER_PIN, 1319, 800); break; // E6 - finale!
            }
            musicNote++;
            if (musicNote > 12) musicNote = 12;  // Stop after finale
        }
    ...
    if (inMenuMode) {
        ...
        if (isBtnUp()) {
            if (now - lastUpPress >= 150) {
                ...
                if (inToolsSubMenu) {
                    ...
                    toolsSelection = (toolsSelection > 1) ? toolsSelection - 1 : 3;
        ...
        if (isBtnDown()) {
            if (now - lastDownPress >= 150) {
                ...
                if (inToolsSubMenu) {
                    oldSel = toolsSelection;
                    toolsSelection = (toolsSelection < 3) ? toolsSelection + 1 : 1;
                    newSel = toolsSelection;
                    updateMenuSelection(oldSel, newSel);
                } else if (inSymmetricSubMenu) {
                    oldSel = toolsSelection;
                    toolsSelection = (toolsSelection < 2) ? toolsSelection + 1 : 1;
                    newSel = toolsSelection;
                    updateMenuSelection(oldSel, newSel);
                } else if (inSymmetricSubMenu) {
                    oldSel = symmetricSizeSelection;
